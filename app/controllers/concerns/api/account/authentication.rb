...
module Api::Account::Authentication
  ...
  def authenticate
    ## Check for the existence and format of the Authorization header. (Authorizationヘッダーの存在とフォーマットをチェック)
    # authorization_header = request.headers["Authorization"]
    ## An error occurs if the header is missing or does not start with "Bearer ". (ヘッダーがない、または "Bearer " で始まっていない場合はエラー)
    ...
    ## Extract the token string (トークン文字列を抽出)
    ...
    begin
      ...
      # If the user ID in the token does not correspond to an existing user (トークン内のユーザーIDが既存のユーザーに対応しない場合)
      raise Api::UnauthorizedError.new("提供されたトークンに対応するユーザーが見つかりません。")
    rescue JWT::ExpiredSignature
      # If the token's 'exp' (expiration) claim indicates that it has expired (トークンの 'exp' (有効期限) クレームが期限切れを示している場合)
      raise Api::UnauthorizedError.new("認証トークンの有効期限が切れています。")
    rescue JWT::DecodeError => error
      # General JWT decoding failure (e.g., invalid signature, malformed token structure) (一般的なJWTデコード失敗 (例: 無効な署名、不正なトークン構造))
      Api::Account::Authentication.error_log("JWT Decode Error: #{error.message}")
      raise Api::UnauthorizedError.new("認証トークンが無効です。")
    rescue => exception
      # Catch other unexpected errors during the authentication process. (認証プロセス中のその他の予期せぬエラーを捕捉)
